...
func main() {
	...
	outsideServer := outsideserver.New(os.Getenv("SONGLIBRARY_OUTSIDE_SERVER_BIND_ADDRESS"))
	//Подключение бизнес логики
	service := service.New(store, outsideServer)
	//Конфигурация http сервера
	router := gin.New()
	router.Use(gin.Recovery())
	router.Use(middlewares.LoggingMiddleware())
	controller.InitController(router, service)
	server := http.Server{
